...
def get_employee_report():
    ...
    for emp in o.EMPLOYEES:
        report += "\"" + str(emp.id) + "\",\"" + emp.name + "\",\"" + emp.code + "\"\n"
    today = datetime.datetime.now().strftime('%Y-%m-%d')
    filename = str(today) + "--" + "员工信息表"
    io.create_CSV(filename, report)
...
# 获取某天所有员工的打卡记录信息
def get_record_today(someday):
    record = "\"员工id\",\"员工姓名\",\"第一次打卡时间\",\"第二次打卡时间\"\n"
    for item in o.LOCK_RECORD:
        record += "\"" + str(item) + "\","
        for emp in o.EMPLOYEES:
            if item == str(emp.id):
                record += "\"" + emp.name + "\","
        for dateTime in o.LOCK_RECORD[item]:
            print(dateTime)
            ...
            if date == someday:
                record += "\"" + str(time) + "\","
        record += "\n"
    filename = str(someday) + "---" + "所有员工打卡记录表"
    io.create_CSV(filename, record)


# 为指定员工添加打卡记录
def add_lock_record(id):
    record = o.LOCK_RECORD  # 所有打卡记录
    now_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")  # 当前时间
    if id in record.keys():  # 如果这个人有打卡记录
        r_list = record[id]  # 获取他的记录
    else:  # 如果这个人从未打过卡
        r_list = list()  # 创建新列表
    r_list.append(now_time)  # 记录当前时间
    record[id] = r_list  # 将记录保存在字典中
    o.LOCK_RECORD = record  # 对字典进行更新
    io.save_lock_record(now_time, id)  # 持久化保存新增打卡记录
...
def get_day_report(date):
    ...
    record = "\"应到人数：\",\"" + str(emp_count) + "\"缺席人数：\",\"" + str(len(absent_list)) + "\"\n"
    absent_name = "\"缺席名单：\"\n\""  # 缺席名单
...
def save_work_time(work_time, close_time):
    ...
    io.save_work_time_config(work_time, close_time)  # 上下班时间保存到文件中
